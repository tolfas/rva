...
public class StoreContentFrameWidget extends RpcDialogBoxWidget {
	private static StoreContentFrameWidget instance;
	...
	public static StoreContentFrameWidget getInstance() {
		try {
			if (instance == null) {
				instance = new StoreContentFrameWidget();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public StoreContentFrameWidget() {
		super();
			...
			GadgetDataController.getInstance().getGadgets(gadgetsInfo, shared, new AsyncCallback<GadgetsInfo>() {
				...
				public void onSuccess(GadgetsInfo result) {
					if (result.getGadgets().size() > 0) {
						...
						StoreContentFrameWidget.this.onFailure("No Gadget associated with productCode: " + productCode);
						onWidgetCancel();
						hide();
					}
				}
				...
				public void onFailure(Throwable caught) {
					StoreContentFrameWidget.this.onFailure(StatusBoxWidget.RPC_ERROR);
					onWidgetCancel();
					hide();
				}
			});
